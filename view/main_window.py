...
class MainWindow(QMainWindow):
    ...
    def action_open(self):
        ...
        filename, _ext = dialog.getOpenFileName(
            self, "Open file", last_path, "json(*.json)")
        if filename == "":
            return
    ...
    def action_save(self):
        if self.graphicsView.file_name is None:
            dialog = QFileDialog()
            last_path = g.config.data['last_project']
            filename, _ext = dialog.getSaveFileName(
                self, "Save file", last_path, "json(*.json)")
            if filename == "":
                return
    ...
    def send_cmd(self, cmd, content=""):
        ...
        self.udpSocket.writeDatagram(
            bytesData, QHostAddress("127.0.0.1"), 9091)
